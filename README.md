...
1.	配置以下服务的VPC端点：
o	com.amazonaws.[区域].ssm
o	com.amazonaws.[区域].ec2messages
o	com.amazonaws.[区域].ssmmessages
2.	确保这些VPC端点有适当的安全组规则
3.	验证IAM权限：
o	实例必须有带有适当SSM权限的IAM角色
o	尝试连接的用户需要Session Manager权限
...
5. 配置设置：
   o	服务类别：AWS服务
   o	服务名称：com.amazonaws.us-west-2.s3
   o	VPC：选择您的VPC（例如：vpc-04fe37b08c6273a9e）
   o	路由表：选择包含无公网出口子网的路由表
...
9. 配置设置：
   o	服务类别：AWS服务
   o	服务名称：com.amazonaws.us-west-2.s3（注意选择接口类型的服务）
   o	VPC：选择您的VPC（例如：vpc-04fe37b08c6273a9e）
   o	子网：选择无公网出口的子网
   o	启用DNS名称：勾选此选项（重要！）
   o	安全组：选择或创建允许HTTPS流量的安全组

10. 点击"创建端点"

11. 记录端点ID（例如：vpce-0b0354085ac5093f4）

    ### 3.5 通过ACL Enable公开桶，并且配置ip访问白名单；
